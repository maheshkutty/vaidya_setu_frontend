  ...
  const formatRecord = async () => {
    try {
      let data = {
        ...
        name,
        desc,
        diagonisReport: await Promise.all(
          dReportFiles.map(async (item) => {
            let cid = await Web3StorageApiPut([item.fileObj]);
            return cid;
          })
        ),
        treatmentReport: await Promise.all(
          tReport.map(async (item) => {
            let cid = await Web3StorageApiPut([item.fileObj]);
            return cid;
          })
        ),
        drugsReport: await Promise.all(
          drugsReport.map(async (item) => {
            let cid = await Web3StorageApiPut([item.fileObj]);
            return cid;
          })
        ),
    ...
    <div>
      ...
      >
        ...
        <AccordionDetails>
          <DrugsForm
            dReport={drugsReport}
            setDreport={setDrugsReport}
            drugsName={drugsName}
            setDrugsName={setDrugsName}
          />
        </AccordionDetails>
      </Accordion>
      <Button
